...
export function ThemeToggle() {
  ...
  return (
    <button
      ...
      className={`relative w-14 h-7 rounded-full transition-colors duration-[1000ms] focus:outline-none focus:ring-2 bg-[var(--border-color)] ${transitioning ? "pointer-events-none" : ""}`}
    ...
    >
      ...
      >
        {theme === "dark" ? (
          <svg className={`w-3.5 h-3.5 text-white ${transitioning ? "animate-spin" : ""}`} style={transitioning ? { animationDuration: "2s" } : {}} fill="currentColor" viewBox="0 0 20 20">
            <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
          </svg>
        ) : (
          <svg className={`w-3.5 h-3.5 text-amber-600 ${transitioning ? "animate-spin" : ""}`} style={transitioning ? { animationDuration: "2s" } : {}} fill="currentColor" viewBox="0 0 20 20">
